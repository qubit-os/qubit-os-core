...
def validate_hermitian(
    ...
    if max_deviation > tolerance:
        ...
        warnings.append(
            f"{name} Hermiticity: deviation {max_deviation:.2e} is close to tolerance"
        )
...
def validate_unitary(
    ...
    if max_deviation > tolerance:
        ...
        warnings.append(
            f"{name} unitarity: deviation {max_deviation:.2e} is close to tolerance"
        )
...
def validate_pulse_envelope(
    ...
    if len(envelope) != num_time_steps:
        errors.append(
            f"{name} length {len(envelope)} != expected {num_time_steps}"
        )
    ...
    if max_val > max_amplitude:
        errors.append(
            f"{name} max amplitude {max_val:.2f} exceeds limit {max_amplitude:.2f}"
        )
...
def validate_calibration_t1_t2(
    t1_us: float,
    t2_us: float
) -> ValidationResult:
    ...
    if t2_us > 2 * t1_us:
        errors.append(
            f"T2 ({t2_us} us) > 2*T1 ({2*t1_us} us) violates physics constraint"
        )
    
    # Typically T2 < T1 in real systems
    if t2_us > t1_us:
        warnings.append(
            f"T2 ({t2_us} us) > T1 ({t1_us} us) is unusual - verify calibration"
        )
...
class AgentBibleValidator:
    ...
    def __init__(self) -> None:
        ...
        if not self._ab_available:
            logger.warning(
                f"AgentBible not available: {_agentbible_import_error}. "
                "Using fallback validators."
            )
    ...
    def validate_pulse(
        ...
        # Validate I envelope
        i_result = validate_pulse_envelope(
            i_envelope, max_amplitude, num_time_steps, "I envelope"
        )
        errors.extend(i_result.errors)
        warnings.extend(i_result.warnings)
        
        # Validate Q envelope
        q_result = validate_pulse_envelope(
            q_envelope, max_amplitude, num_time_steps, "Q envelope"
        )
...
def validate_pulse(
    ...
    return default_validator.validate_pulse(
        i_envelope, q_envelope, max_amplitude, num_time_steps
    )
...
def validate_calibration(
    ...
    return default_validator.validate_calibration(
        t1_us, t2_us, readout_fidelity, gate_fidelity
    )
